...
def break_sentences(sentences, speech_verbs):
    ...
    for sent in sentences:
        ...
        if sent.text.startswith("“") and sent.text.endswith("”") and sent.text.count("“") == 1 and sent.text.count("”") == 1:
            broken_sentences.append(sent.text)
            continue
        ...
        for i, token in enumerate(tokens):
            if token.text in speech_verbs:
                for j in range(i + 1, len(tokens)):
                    if tokens[j].text == "“":
                        break_points.append(j-1)  # 在逗号位置添加断点
                        break
        ...
        if break_points:
            ...
            for point in break_points:
                broken_sentences.append(sent[start:point+1].text)
                start = point + 1
            broken_sentences.append(sent[start:].text)
        else:
            broken_sentences.append(sent.text)
...
# sentencesMerged = merge_sentences(sentences, 5)
